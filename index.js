...
console.log (Profile[1]['personel Numarası']);
...
function indekseGoreFenomen(istenilenDizi,istenilenSayı) {

return istenilenSayı+'. indekste bulunan fenomen: '+istenilenDizi[istenilenSayı].profile;
}

console.log(indekseGoreFenomen(fenomenler,19));
...
function profilListesi(gelenDizi) {
...
function fenomenSil(gelenDizi,gelenSayı) {
  let yeniDizi = [].concat(gelenDizi);
  yeniDizi.splice(gelenSayı,1) ; return yeniDizi;
...
function fenomenEkle(gelenDizi,gelenNumber,gelenProfile,gelenFollowers,gelenPosts,gelenPlatform) {
  let yeniDizi = [...gelenDizi];
let yeniObje = {
  "number" : gelenNumber,
...
function enFenomenler(gelenDizi) {
let yeniDizi = [];
for (let i = 0 ; i < gelenDizi.length; i++){
  if (gelenDizi[i].followers > 100000000){
    yeniDizi.push(gelenDizi[i].profile);
  }
}
return yeniDizi;  
}
...
function fenomenGonderimSayisi(GelenDizi,GelenProfile){
let arananKişi = GelenDizi.filter((indeks) => {
 return indeks.profile === GelenProfile;
}
);
return arananKişi[0].posts ;
}
...
function platformaGoreCokGonderiYapanFenomen(GelenDizi,GelenMecra){
let maksPostSayısı = 0;
let fenomenİsmi = '';
for ( let i = 0 ; i < GelenDizi.length ; i++){
  if ( GelenDizi[i].platform === GelenMecra){
    if ( maksPostSayısı < GelenDizi[i].posts){
      maksPostSayısı = GelenDizi[i].posts;
      fenomenİsmi = GelenDizi[i].profile;
    }
  }
}
return fenomenİsmi}
